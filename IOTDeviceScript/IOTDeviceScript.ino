...
const char ssid[] = "MEJIA"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "123456789"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "52.23.200.41"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
...
const char MQTT_TOPIC_PUB[] = "colombia/narino/pasto/" USER "/out"; //TODO Reemplace el valor por el tópico de publicación que le corresponde.
// El tópico de suscripción debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/in
const char MQTT_TOPIC_SUB[] = "colombia/narino/pasto/" USER "/in"; //TODO Reemplace el valor por el tópico de suscripción que le corresponde.
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (data.indexOf("ALERT") >= 0) {
    alert = data;
  } else if (data.indexOf("FIRE") >= 0){
    Serial.print("\nAlarma de fuego registrada");
...
void measure() {
  if ((millis() - measureTime) >= MEASURE_INTERVAL * 1000 ) {
    Serial.println("\nMidiendo variables...");
    Serial.print("\nMediciones: ");
    Serial.print(fire_count_hack);
    Serial.print("\n");
    fire_count_hack = fire_count_hack + 1;
